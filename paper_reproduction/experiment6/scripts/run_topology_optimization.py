...
part.SectionAssignment(
    region=part.Set(cells=part.cells, name="AllCells"), sectionName="Section"
)
...
model.StaticStep(
    name="LoadStep", previous="Initial", initialInc=1.0, maxInc=1.0, minInc=1e-6
)
model.FieldOutputRequest(
    name="F-Output-1", createStepName="LoadStep", variables=("S", "U", "RF", "ENER")
)

# --- BCs and Load ---
print("[5/8] Applying BCs and load...")
model.EncastreBC(
    name="Fixed", createStepName="Initial", region=assembly.sets["FixedFace"]
)
model.Pressure(
    name="Load",
...
model.optimizationTasks["TopoTask"].ObjectiveFunction(
    name="MinSE", objectives=((OFF, "strain_energy", 1.0, 0.0, ""),), target=MINIMIZE
)
...
mdb.Job(name="SimpleTO_FEA", model="SimpleTO", numCpus=NUM_CPUS, numDomains=NUM_CPUS)
...
# IOWarp.saveAs(cae)
#    mdb.saveAs('SimpleTO.cae')
# intercepts_the_write (LD_PRELOAD trick)
#    LD_PRELOAD=IOWARP_INTERCEPTOR.so abaqus
#    IOWARP_INTERCEPTOR.so
#           Write() -> Writes to the bucket with the file_name
#           Read()
#           Open()  -> Creates Buckets
